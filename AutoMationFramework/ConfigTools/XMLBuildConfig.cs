...
namespace ConfigTools
{
    public class XmlBuildConfig : IBuildConfig
    {
        ...
        {
            int starNum = fileName.LastIndexOf("\\", StringComparison.Ordinal) + 1;
            int nums = fileName.Length - starNum;
            string file = fileName.Substring(starNum, nums);
            switch (file)
            {
                case "systemCfg":
        ...
        public void SaveAsConfig<T>(T tCfg)
        {
            ...
            if (result == true)
            {
                ...
                int nums = file.LastIndexOf(".", StringComparison.Ordinal);
                file = file.Substring(0, nums);
