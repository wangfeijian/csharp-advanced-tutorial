...
namespace FinsDebuger
{
    public partial class Form1 : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show($"获取指定位状态成功：当位状态为 {Convert.ToString(bs,2).PadLeft(16,'0')}！");
            }
        }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            short re = ENT.ReadString(PlcMemory.Dm, short.Parse(address), short.Parse(count), out rd);
            if (re != 0)
            {
                //写入数据出错
                textBox3.Text = "数据读取出错！";
            }
            else
            {
                textBox3.Text = rd;
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            string address = textBox2.Text.Trim();
            string count = textBox1.Text.Trim();
            short re = ENT.WriteString(PlcMemory.Dm, short.Parse(address), textBox3.Text.Trim());
            if (re != 0)
            {
                //写入数据出错
                textBox3.Text += ",写入数据出错！";
            }
            else
            {
                textBox3.Text += ",写入数据成功！";
            }
        }
    }
}
